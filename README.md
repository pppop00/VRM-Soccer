...
给定一个 clip 的 ground_truth.mp4（22 个彩色圆点在绿色球场上运动），V2V 模型生成对应的预测视频后，`scorer.py` 从五个维度自动打分，全部基于规则计算，不调用 MLLM（ICLR 2025 研究证明 rule-based 与人类偏好 Spearman > 0.9，优于 MLLM 评判）。

$$S_{total} = w_1 D1 + w_2 D2 + w_3 D3 + w_4 D4 + w_5 D5$$

默认权重各 0.2，可通过 `--weights` 覆盖。VBVR 学术对齐：D1 → Perception，D2 → Transformation，D3/D4 → Knowledge，D5 → Abstraction。

---

### D1 · Visual Quality — 点在不在？

**问题：生成视频里各帧的球员/球位置，和 ground truth 差多少？**

逐帧计算 SSIM（结构相似度）。球在 (62m, 34m)，模型生成在 (61m, 35m)，SSIM ≈ 0.94；跑偏到 (52m, 50m) 则 SSIM ≈ 0.41。

升级路径：SSIM → DINOv2 跨帧特征相似度（VBench CVPR 2024）。

---

### D2 · Temporal Consistency — 移动是否流畅？

**问题：相邻两帧之间，圆点的移动是否连贯，有没有闪烁/跳变？**

用光流 warping error 衡量帧间连续性。away_3 从 (30m, 20m) 一帧跳到 (55m, 40m)（瞬移 25m），warping error 飙高 → D2 扣分。

升级路径：Farneback 光流 → RAFT（ECCV 2020）。

---

### D3 · Physical Plausibility — 速度合不合理？

**问题：圆点的运动速度是否超出物理上限？球会不会飞出场地？**

从每帧 HSV 颜色提取坐标 → 换算米单位 → 计算帧间速度：

| 检查项 | 阈值 | 违规示例 |
|--------|------|---------|
| 球员速度 | > 10 m/s | away_3 帧间 160 m/s |
| 球速度 | > 35 m/s | 球帧间 288 m/s |
| 坐标出界 | x ∉ [0,105] 或 y ∉ [0,68] | 球出现在看台 |
| 轨迹跳变 | 球员 > 5m/帧，球 > 10m/帧 | 瞬移检测 |

输出违规明细：`{frame: 46, agent: "ball", type: "speed", value: 288}`。

待扩展：加入 WCS Object Permanence——用 ByteTrack 追踪球消失后是否在合理位置重现（arxiv 2508.00144）。

---

### D4 · Tactical Coherence — 22 个点的关系对不对？

**问题：谁控球？进攻方向对不对？队友支援结构是否合理？**

- **持球一致性**：球距离哪支队伍最近，判定持球方；检查是否与 prompt.txt 描述一致
- **Support Ratio**：持球球员 20m 内队友数量 / 总队友数，衡量进攻支援结构
- **Flicker Penalty**：持球方在短时间内反复翻转（主队→客队→主队）扣分

```
ground truth：主队持续控球，support_ratio = 0.4
模型生成：第55帧起判定客队持球，但 prompt 写"主队推进" → D4 扣分
```

---

### D5 · Prompt Adherence — 和文字描述一致吗？

**问题：生成视频的整体画面，是否符合 prompt.txt 描述的战术场景？**

取生成视频关键帧 → 提取 CLIP embedding → 与 prompt.txt 文本计算余弦相似度：

```
prompt.txt："主队从左路推进，7号带球，攻击阶段"
主队在左路进攻的画面 → similarity ≈ 0.31  ✅
双方在中场拉锯的画面 → similarity ≈ 0.18  ❌
```
...
python scorer.py \
  ...
  --weights '{"D1":0.2,"D2":0.1,"D3":0.3,"D4":0.2,"D5":0.2}'
```

输出示例：

```
D1 Visual Quality:        0.68
D2 Temporal Consistency:  0.45
D3 Physical Plausibility: 0.30  [violations: 3 frames]
D4 Tactical Coherence:    0.50
D5 Prompt Adherence:      0.72
─────────────────────────
S_total:                  0.53
```
...
| P1 | D3 扩展：WCS Object Permanence（ByteTrack） | `bytetrack` | ~3 天 |
| P2 | D6：轨迹精度 Fréchet / ADE / FDE | `scipy` | ~2 天 |
| P2 | D7：战术阵型 EFPI matching | `unravelsports` | ~2 天 |
| P3 | D8：Pitch Control 表面 EMD | `LaurieOnTracking` | ~3 天 |
| Phase 3 | 深度图一致性（广播视角接入后） | `depth-anything` | — |
